...
完整 CI 34612008991（源 `09769cb8f31290a5096f2411e3a35a57a22daae1`）success：KMP、StoreKit 9/9、UI 1/1（484.617 秒）、无签名设备 Archive、arm64/x86_64 Simulator、安装冷启动及元数据/崩溃扫描通过。下载 ZIP SHA-256 `1EC017A4AD44567856F8AEFAFFDFF41A93276A26D219CB99DB86C39621970D7C` 与 CI 一致，40,803,435 bytes。Android APK 哈希复核未变，沿用本轮已通过的 JVM/31 Room/21 Compose/Lint/构建/签名证据。下方“待验证”段落保留为修复过程历史，已由此结果闭环。最后只回填文档，不改变已验收代码。真机签名与 App Store 验收仍不在此通过结论内。

### iOS 首次验收修复静态门禁

34609224131：引导、Pin 与提醒、订阅、高余额免费锁、逐字清空与编辑、兑换无赠币、物品使用/出售、月任务均已推进通过，失败在 Files 首次枚举的 15 秒文件等待。14:35:57 打开选择器，14:36:13 最后检查，14:36:22 失败层级已包含可用 `quest-backup-2026-09-11.json, json` Cell，截图也显示已导出的文件；并非导出丢失。✅ 仅将测试选择器定位改为文件稳定 identifier 前缀，并将首次目录枚举等待限定为 30 秒；保留文件启用断言、实际点击、Backup imported 与恢复任务断言，不改生产文件/恢复代码或全局超时。静态门禁通过后完整 CI 重跑，导入链路仍待本轮实测。

34607242034：StoreKit 9/9 通过，UI 在引导结束检查失败，未执行到逐字删除验证。日志点击 Skip StaticText 后，截图与语义树仍显示引导第 4 页，实际 `first-run-skip` Button 位于 x=311,y=59,width=58,height=48。✅ 本轮测试改为点击该实际按钮 frame 中心，并新增等待引导控件消失的断言；不再依赖嵌套虚拟 StaticText 的推断点击点，不重试跳过、不删除后续空愿望断言。生产代码/架构/Android APK 无变化。静态门禁通过后运行完整 iOS CI，点击点问题的修复效果与逐字删除仍待验证。

34604798798：StoreKit 9/9 通过，首次 Pin/提醒/余额与模拟订阅已通过，UI 在编辑任务清空名称时失败。日志发送 16 个批量 delete 后，截图与语义树均仍为 `Acce`，光标在末尾；不是旧弹窗问题。✅ 本轮仅改 XCUITest 输入驱动为每发一个 delete 即等待确切的末尾字符删除，再发下一键，次数限制为原文本长度；仍强制清空后才输入替换文本，保留精确新值断言。未改生产代码/数据/API，静态门禁通过后重新执行完整 CI；Android 产物不变。批量按键与 Compose 文本更新的时序问题由该修复验证，尚不宣称通过。
